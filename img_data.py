        ...
        def save_train_dataset(train_li=train, mode="train"):
            ...
            for i, folder_name in enumerate(classes):
                ...
                for k in range(train_li[i]):
                    ...
                    coord = func()
                    save_wafer_plot(coord, f"{path}/{mode}/{folder_name}/{k+1 + num:05d}.png")
                    plt.close()
        ...
        def save_train_dataset(n=train, mode="train"):
            ...
            for i, folder_name in enumerate(classes):
                for k in range(1, n+1):
                    ...
                    func = globals()[function_name]
                    coord = func()
                    save_wafer_plot(coord, f"{path}/{mode}/{folder_name}/{k + n * i:05d}.png")
                    plt.close()
    ...
    def save_val_dataset(num=train_num, n=val, mode="valid"):
        ...
        for i, folder_name in enumerate(classes):
            for k in range(1, n+1):
                ...
                func = globals()[function_name]
                coord = func()
                save_wafer_plot(coord, f"{path}/{mode}/{folder_name}/{train_len + k + n * i:05d}.png")
                plt.close()
    ...
    def save_test_dataset(num=train_val_num, n=test, mode="test"):
        ...
        for i, folder_name in enumerate(classes):
            for k in range(1, n+1):
                ...
                func = globals()[function_name]
                coord = func()
                save_wafer_plot(coord, f"{path}/{mode}/{folder_name}/{num + k + n * i:05d}.png")
                plt.close()
...
if __name__=='__main__':
    parser = argparse.ArgumentParser('Set images Datasets number script', parents=[get_args_parser()])
